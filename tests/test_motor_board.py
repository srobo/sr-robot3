...
from sr.robot3.motor_board import MotorBoard, MotorPower
...
def motorboard_serial(monkeypatch) -> None:
    ...
    monkeypatch.setattr('sr.robot3.motor_board.atexit', mock_atexit)
    monkeypatch.setattr('sr.robot3.motor_board.SerialWrapper', serial_wrapper)
...
def test_motor_board(motorboard_serial: MockMotorBoard) -> None:
    ...
    assert motorboard.motors[0].is_in_fault() is False
    assert motorboard.motors[1].is_in_fault() is True
...
def test_motor_board_motors(motorboard_serial: MockMotorBoard) -> None:
    ...
    motorboard.motors[0].set_power(0.5)
    motorboard.motors[1].set_power(0.5123)

    # Test that we can disable the motors
    motorboard.motors[0].set_power(MotorPower.COAST)
    motorboard.motors[1].set_power(MotorPower.COAST)

    # Test setting the power to BRAKE
    motorboard.motors[0].set_power(MotorPower.BRAKE)
    motorboard.motors[1].set_power(MotorPower.BRAKE)

    # Test that we can get the motor power
    assert motorboard.motors[0].get_power() == 0.123
    assert motorboard.motors[1].get_power() == 0.456

    # Test that we can get the motor enabled state
    assert motorboard.motors[0].get_power() == MotorPower.COAST
    assert motorboard.motors[1].get_power() == MotorPower.COAST

    # Test that we can get the motor current
    assert motorboard.motors[0].get_current() == 1.234
    assert motorboard.motors[1].get_current() == 12.345
...
def test_motor_board_bounds_check(motorboard_serial: MockMotorBoard) -> None:
    ...
    with pytest.raises(TypeError):
        motorboard.motors['a'].set_power(0.5)
    with pytest.raises(TypeError):
        motorboard.motors[0.5].set_power(0.5)
    with pytest.raises(TypeError):
        motorboard.motors[None].set_power(0.5)

    # Test that we handle invalid power values correctly
    with pytest.raises(ValueError):
        motorboard.motors[0].set_power(-1.2)
    with pytest.raises(ValueError):
        motorboard.motors[0].set_power(1.2)
    with pytest.raises(ValueError):
        motorboard.motors[0].set_power(100)

    # Test that we handle invalid power value types correctly
    with pytest.raises(TypeError):
        motorboard.motors[0].set_power('a')
    with pytest.raises(TypeError):
        motorboard.motors[0].set_power(None)
    with pytest.raises(TypeError):
        motorboard.motors[0].set_power([0.5])
...
def test_motor_board_discovery(monkeypatch) -> None:
    ...
    monkeypatch.setattr('sr.robot3.motor_board.SerialWrapper', serial_wrapper)
    monkeypatch.setattr('sr.robot3.motor_board.comports', mock_comports)
